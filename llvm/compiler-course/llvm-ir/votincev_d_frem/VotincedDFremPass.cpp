      ...
      for (auto &instr : llvm::make_early_inc_range(bb)) {
        if (auto *binOp = llvm::dyn_cast<llvm::BinaryOperator>(&instr)) {
          ...
          if (binOp->getOpcode() == llvm::Instruction::FRem) {
            ...
            auto *op1 = builder.CreateFDiv(lhs,rhs); // a/b (обычное деление fp)
            op1 = builder.CreateUnaryIntrinsic(llvm::Intrinsic::trunc,op1); // округление a/b
            ...
            auto *op1 = builder.CreateUDiv(lhs,rhs); // a/b (обычное целочисленное деление fp)
            ...
            auto *op1 = builder.CreateSDiv(lhs,rhs); // a/b (обычное целочисленное деление fp)
            auto *op2 = builder.CreateMul(op1,rhs); // (a/b) * b
